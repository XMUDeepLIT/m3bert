...
class MatryoshkaDistillationLoss(nn.Module):
    def __init__(
        self,
    ...
    def forward(self, sentence_features: Iterable[dict[str, Tensor]], labels: Tensor) -> Tensor:
        ...
        try:
            ...
            loss = 0.0
            teacher_cl_logits_list = []
            for idx in dim_indices:
                dim = self.matryoshka_dims[idx]
                weight = self.matryoshka_weights[idx]
                decorated_forward.set_dim(dim)
                cl_loss, cl_logits = self.loss(sentence_features, labels)
                loss += weight * cl_loss
                
                distillation_loss = 0.0
                if self.teacher_idxs:
                    cur_teachers=[l for i,l in enumerate(teacher_cl_logits_list) if i in self.teacher_idxs[idx]]
                    for teacher_cl_logits in cur_teachers:
                        distillation_loss += nn.functional.kl_div(cl_logits.log_softmax(dim=-1), teacher_cl_logits.softmax(dim=-1).detach(), reduction='batchmean')
                    if len(cur_teachers) > 0:
                        distillation_loss = distillation_loss / len(cur_teachers)
                        loss += weight * distillation_loss
                else:
                    for teacher_cl_logits in teacher_cl_logits_list:
                        distillation_loss += nn.functional.kl_div(cl_logits.log_softmax(dim=-1), teacher_cl_logits.softmax(dim=-1).detach(), reduction='batchmean')
                    if len(teacher_cl_logits_list) > 0:
                        distillation_loss = distillation_loss / len(teacher_cl_logits_list)
                        loss += weight * distillation_loss

                teacher_cl_logits_list.append(cl_logits)
        finally:
