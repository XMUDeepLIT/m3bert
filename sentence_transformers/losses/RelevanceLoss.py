...
class RelevanceLoss(nn.Module):
    def __init__(self, model: SentenceTransformer, sentence_embedding_dimension: int, num_labels: int) -> None:
        """
        ...
        self.transform = nn.Linear(self.sentence_embedding_dimension*2, self.sentence_embedding_dimension*2)
        self.classifier = nn.Linear(self.sentence_embedding_dimension*2, self.num_labels)
        self.cross_entropy_loss = nn.CrossEntropyLoss()

    def forward(self, sentence_features: Iterable[dict[str, Tensor]], labels: Tensor) -> Tensor:
        reps = [self.model(sentence_feature)["sentence_embedding"] for sentence_feature in sentence_features]
        embeddings_query, embeddings_document = reps
        concat_embeddings = torch.cat([embeddings_query, embeddings_document], dim=-1)
        
        shrunk_dim = concat_embeddings.shape[-1]
        hidden_states = nn.functional.linear(concat_embeddings, self.transform.weight[:shrunk_dim, :shrunk_dim], self.transform.bias[:shrunk_dim])
        logits = nn.functional.linear(hidden_states, self.classifier.weight[:shrunk_dim, :shrunk_dim], self.classifier.bias[:shrunk_dim])
    ...
    @torch.no_grad()
    def inference(self, embeddings_query, embeddings_document):
        concat_embeddings = torch.cat([embeddings_query, embeddings_document], dim=-1)
