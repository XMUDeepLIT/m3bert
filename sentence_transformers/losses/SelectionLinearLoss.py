...
class SelectionLinearLoss(nn.Module):
    ...
    def forward(self, embeddings_query, embeddings_document, labels: Tensor, inbatch=True) -> Tensor:
        # # print(f"sentence_features[0]={sentence_features[0]}")
        # reps = [self.model(sentence_feature)["sentence_embedding"] for sentence_feature in sentence_features]
        # # print(f"reps = {reps}")
        # embeddings_query, embeddings_document = reps
        # todo:linear应该加在这里
        # embeddings_query = nn.functional.normalize(embeddings_query, p=2, dim=-1)
        # embeddings_document = nn.functional.normalize(embeddings_document, p=2, dim=-1)

        logits_per_query = torch.matmul(embeddings_query, embeddings_document.t()) * self.scale
        # logits_per_document = logits_per_query.t()

        batch_size = embeddings_query.size(0)
        if inbatch:
            labels = torch.arange(batch_size, dtype=torch.long).cuda()

        loss_query = self.cross_entropy_loss(logits_per_query, labels)
        # loss_document = self.cross_entropy_loss(logits_per_document, labels)

        return loss_query, logits_per_query
