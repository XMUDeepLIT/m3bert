    ...
    def __call__(
        ...
        for fn_name in self.similarity_fn_names:
            if fn_name in similarity_functions:
                ...
                # doc_index = faiss.IndexIVFPQ(faiss.IndexFlatIP(d), d, 100, d // 16, 8)
                # gpu_doc_index = faiss.index_cpu_to_gpu(res, 0, doc_index)
                # gpu_doc_index.train(documents)

                # doc_index = faiss.IndexPQ(d, d // 8, 8, faiss.METRIC_INNER_PRODUCT)
                # gpu_doc_index = faiss.index_cpu_to_gpu(res, 0, doc_index)
                # gpu_doc_index.train(documents)
                ...
                for i, (query, label_document) in enumerate(zip(queries, labels)):
                    ...
                    # correct_doc_idx = i
                    flag = False
                    for j, doc_index in enumerate(indices[i][:10]):
                    ...
                    # rank = np.where(indices[i][:k] == correct_doc_idx)[0]
