...
def encode_on_multiple_gpus(model, data, num_gpus, batch_size=1024):
    ...
    with ThreadPoolExecutor(max_workers=num_gpus) as executor:
        ...
        for future in futures:
            ...
            results.append(result.cpu())  # 搬到 CPU
            del result  # 删除 GPU 张量
...
class MRRMultiGPUEvaluator(SentenceEvaluator):
    def __init__(
        self,
        sentences1: list[str],
        sentences2: list[str],
    ...
    def __call__(
        ...
        with nullcontext() if self.truncate_dim is None else model.truncate_sentence_embeddings(max_dim):
            ngpus = faiss.get_num_gpus()
            print("number of GPUs:", ngpus)

            logger.info(f"encoding 1")
            if self.len1 is not None:
                model.max_seq_length = self.len1
                print(f"max_len = {model.max_seq_length}")
            embeddings1_ori = encode_on_multiple_gpus(model, self.sentences1, num_gpus=ngpus, batch_size=self.batch_size)
            ...
            if self.len2 is not None:
                model.max_seq_length = self.len2
                print(f"max_len = {model.max_seq_length}")
            embeddings2_ori = encode_on_multiple_gpus(model, self.sentences2, num_gpus=ngpus, batch_size=self.batch_size)
        ...
        for mrl_dim in mrl_dims:
                    ...
                    for mrr_len in mrr_lens:
                        ...
                        for i, (query, label_document) in enumerate(zip(queries, labels)):
                            ...
                            for j, doc_index in enumerate(indices[i][:mrr_len]):
                                if label_document[:512] == self.sentences2[doc_index][:512]:
                                    rr_sum += 1.0 / (j + 1)
                                    flag = True
                                    break
                        ...
                        metrics[f"{fn_name.capitalize()}-MRR@{mrr_len}-dim{mrl_dim}"] = eval_mrr
                        metrics[f"{fn_name.capitalize()}-Recall@{mrr_len}-dim{mrl_dim}"] = eval_recall
            ...
            if output_path is not None:
                result_path = os.path.join(output_path, "result.jsonl")
